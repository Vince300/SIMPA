...
import simpa.hit.drivers.efsm.RandomEFSMDriver;
import simpa.hit.examples.efsm.RandomEFSM;
			...
			if (Options.RETEST == -1) {
				...
				for (i = 1; i <= Options.NBTEST; i++) {
					...
					try {
						...
						lilearner.learn();
						driver.logStats();
						simpa.hit.learner.efsm.LiConjecture c = lilearner
								.createConjecture();
						c.exportToRawDot();
						LogManager.logLine();
						c.exportToDot();
				...
				try {
					...
					lilearner.learn();
					driver.logStats();
					simpa.hit.learner.efsm.LiConjecture c = lilearner.createConjecture();
					c.exportToRawDot();
					LogManager.logLine();
					c.exportToDot();
				} finally {
