...
public class MealyDriver extends Driver {
		...
		if (ce == null){
			...
			try {
				if (m.isConnex())
					...
					throw new UnableToComputeException("simpa.hit.automata is not connex");
	...
	/**
	 * get a shortest distinguish sequence for an simpa.hit.automata
	 * the computed sequence is not applied to the driver
	 * The two simpa.hit.automata are supposed to be connex.
	 * @param s1 the position in the driver equivalent to s2. If null, the current position is chosen
	 * @param a2 the second simpa.hit.automata
	 ...
	 * @param a2 the second simpa.hit.automata
	 * @return a distinguish sequence for the two simpa.hit.automata starting from their initial states or null if the two simpa.hit.automata are equivalents.
