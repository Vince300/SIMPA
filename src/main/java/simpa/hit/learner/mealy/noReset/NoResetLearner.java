...
import simpa.hit.tools.AdenilsoSimaoTool;
...
public class NoResetLearner extends Learner {
		...
		if (driver instanceof TransparentMealyDriver) {
			if (AdenilsoSimaoTool.isToolPresent()) {
				...
				if (minTraceLength > dataManager.traceSize())
					throw new RuntimeException("error in learning, there is another simpa.hit.automata which produce the same trace");
	...
	/**
	 * 
	 * @param trace omega the global trace of the simpa.hit.automata, will be completed \in (IO)*
	...
	private boolean checkRandomWalk(){
		...
		//Now the two simpa.hit.automata are in same state.
	...
	/**
	 * check if the computed conjecture is equivalent to the given simpa.hit.automata
	 * the control is made by walking in the given simpa.hit.automata in order to follow each transitions and comparing the two outputs and checking that only one state of th conjecture  can be associated to a state of the given simpa.hit.automata.
	 * @param automata a connex simpa.hit.automata
	 * @param currentState the state in the simpa.hit.automata which is supposed to be equivalent to the current state of the driver
	 * @return true if the two simpa.hit.automata are equivalent
	 */
	public boolean checkExact(Mealy automata, State currentState){
		...
		//assigned is a table to associate a FoundState to each state in the given simpa.hit.automata
		...
		while (uncheckedState != null) {		
			...
			//we follow path in driver (the conjecture) and the given simpa.hit.automata
			for (String i : path){
