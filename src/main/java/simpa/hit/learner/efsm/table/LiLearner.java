...
import simpa.hit.automata.efsm.ParameterizedOutputSequence;
import simpa.hit.detection.XSSDetector;
...
public class LiLearner extends Learner {
	...
	/**
	 * Allow the simpa.hit.learner not to send requests for already tested sequences
	 */
		...
		for (ParameterizedInput queryPI : query.sequence) {
			...
			for (int k = 0; k < api.getParameters().size(); k++) {
				if (api.isNdv(k)) {
					...
					/* Looks in the current simpa.hit.automata state the last value
					...
					/* If this symbol was not encountered before, "init" is present in the
					 simpa.hit.automata, and there is no need to send this dummy value
			...
			//Store the state in which the simpa.hit.automata was before executing the PI
