...
#codec        @iter_options = @options ;
        @iter_options = (@options, '-DDEBUG') ;
    }
    else {
        $iter_perf_mode = 1 ;
    ...
    foreach my $inp (0 .. $#{$descrs{$target}}) {
        ...
        foreach $current_sim_idx ( 0 .. $#descr_list ) {
            ...
            if ($empty) {
                ...
                #
                #codec if ($iter_func_mode) {
                #codec  # We extract the frames traces from the stdout
                #codec  #
                #codec  @frame_byte_list = () ;
                #codec  my @frame_bytes_trc = grep(/^frame\s*=[0-9]+,\s*[0-9]+\s*bytes written/, @stdout_lines) ;
                #codec  @stdout_sim         = grep(!/^frame\s*=[0-9]+,\s*[0-9]+\s*bytes written/, @stdout_lines) ;
                #codec  foreach my $frame_byte (@frame_bytes_trc) {
                #codec      @frame_byte = ( $frame_byte =~ /[0-9]+/g) ;
                #codec      if ($#frame_byte == 1) {
                #codec          push (@frame_byte_list, [$frame_byte[0], $frame_byte[1]]) ;
                #codec          #DEBUG print STDOUT "Frame == $frame_byte[0], $frame_byte[1] bytes written.\n" ;
                #codec      }
                #codec      else {
                #codec          print STDERR "Trace information (frame number and num bytes written) is bad.\n" ;
                #codec      }
                #codec  }
                #codec }
				...
				# We compute the normalized value from cycle number
					...
					if ( -f "$run_norm" ) {
						...
						my $status = 0 ;
						my $brk = 0 ;
						my @nres = execute_it_errbrkhdl(sub {$status = shift}, sub {$brk = 1}, @cmd) ;
                ...
                if ($user_break) {
					...
					if (($sim_timeout > 0) and (($exit_value == 30) or ($exit_value == 124)) and ($signal_num == 0)) {
						if ($ok_to_report) {
							$num_fail++ ;
							line_report($REPORT, $target, $descr, '', 'FAIL(exec timeout)', $CURRENT_SIZE, '(not executed)', undef, undef) ;
						}
						else {
							${${@pass_status{$target}}[0]}[0] = 'FAIL' ;
							...
							line_report($REPORT_FP, $target, $descr, '', 'FAIL(exec timeout)', $CURRENT_SIZE, '(not executed)', undef, undef) ;
						}
						next TARGET ;
					}
					else {
            ...
            #codec  # Output comparison
            #codec  #
            #codec  #We search the word pass/fail
            #codec  my @pass_fail = grep(/\b(pass|passed|fail|failed)\b/i, @stdout_lines) ;
            #codec  unless (@pass_fail) {
            #codec      $num_fail++ ;
            #codec      line_report($REPORT, $target, $descr, $in_basename, 'FAIL(no pass/fail)', $CURRENT_SIZE, $CYC, [@params_list], undef, $norm_res) ;
            #codec  }
            #codec  else {
            #codec      my @fails = grep(/\b(fail|failed)\b/i, @pass_fail) ;
            #codec      if (@fails) {
            #codec
            #codec          my @fails_infos_l = () ;
            #codec          foreach my $fail_line (@fails) {
            #codec          my ($fail_info) = ($fail_line =~ /\bfail[^(]*\(([^)]+)\).*/i) ;
            #codec          push(@fails_infos_l, $fail_info) ;
            #codec          }
            #codec          my $fails_info = join(' ', @fails_infos_l) ;
            #codec          $num_fail++ ;
            #codec          line_report($REPORT, $target, $descr, $in_basename, "FAIL($fails_info)", $CURRENT_SIZE, $CYC, [@params_list], undef, $norm_res) ;
            #codec      }
            #codec      else {
            ...
            if ($iter_func_mode and not $empty) {
                ...
                if (@differ) {
                #codec  my $error_byte = $differ[0] ;
                #codec  # DEBUG print "\nERROR_byte == $error_byte \n" ; # DEBUG
                #codec
                #codec  my ($frame_num, $byte_num) = (1, 0) ;
                #codec  if (@frame_byte_list) {
                #codec  ($frame_num, $byte_num) = @{$frame_byte_list[0]} ;
                #codec  shift @frame_byte_list ;
                #codec  }
                #codec
                #codec  SEARCH_FRAME:
                #codec  while (@frame_byte_list) {
                #codec
                #codec  # DEBUG print STDOUT "Frame == $frame_num, $byte_num bytes test.\n" ; # DEBUG
                #codec  my ($next_frame_num, $next_byte_num) = @{$frame_byte_list[0]} ;
                #codec
                #codec  if (( $error_byte <= $next_byte_num ) or ($frame_num > $next_frame_num)) {
                #codec
                #codec      # We skip the traces for the first output file for the next output file
                #codec      SEARCH_LAST_FRAME:
                #codec      while (@frame_byte_list) {
                #codec      if (${$frame_byte_list[0]}[0] == 1) {
                #codec          last SEARCH_LAST_FRAME ;
                #codec      }
                #codec      shift @frame_byte_list ;
                #codec      }
                #codec     last SEARCH_FRAME ;
                #codec  }
                #codec  ($frame_num, $byte_num) = ($next_frame_num, $next_byte_num) ;
                #codec  shift @frame_byte_list ;
                #codec  }
                    if ($ok_to_report) {
                        $num_fail++ ;
                #codec  line_report($REPORT, $target, $descr, $in_basename, 'FAIL(Diff)', $CURRENT_SIZE, $CYC, [@params_list], undef, $norm_res, "Error file $out_file byte $error_byte frame $frame_num") ;
                        line_report($REPORT, $target, $descr, $in_basename, 'FAIL(Diff)', $CURRENT_SIZE, $CYC, [@params_list], undef, $norm_res) ;
                    }
                    else {
                        ${${@pass_status{$target}}[$current_sim_idx]}[$inp+1] = 'FAIL' ;
                        ${${@pass_info{$target}}[$current_sim_idx]}[$inp+1]   = '(Diff)';
                #codec      ${$err_msg{$target}}[$inp+1]     = "Error file $out_file byte $error_byte frame $frame_num" ;
                        ${${@err_msg{$target}}[$current_sim_idx]}[$inp+1]     = '' ;

                #codec      line_report($REPORT_FP, $target, $descr, $in_basename, 'FAIL(Diff)', $CURRENT_SIZE, $CYC, [@params_list], undef, $norm_res, "Error file $out_file byte $error_byte frame $frame_num") ;
                        line_report($REPORT_FP, $target, $descr, $in_basename, 'FAIL(Diff)', $CURRENT_SIZE, $CYC, [@params_list], undef, $norm_res) ;
                    }
            ...
            #codec      # DEBUG print STDOUT "We skip traces for the '$out_file' output file.\n" ; # DEBUG
            #codec      # We skip the traces for the first output file for the next output file
            #codec      if (@frame_byte_list) {
            #codec          shift @frame_byte_list ;
            #codec      }
            #codec      SKIP_FRAMES_THIS_INPUT:
            #codec      while (@frame_byte_list) {
            #codec          if (${$frame_byte_list[0]}[0] == 1) {
            #codec              last SKIP_FRAMES_THIS_INPUT ;
            #codec          }
            #codec          shift @frame_byte_list ;
            #codec      }
                    unless ($no_clean) {
